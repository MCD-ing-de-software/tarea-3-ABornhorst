...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        """
        DataFrame = make_sample_df()
        cleaner = DataCleaner()

        resultado = cleaner.drop_invalid_rows(DataFrame, ["name", "age"])

        self.assertEqual(resultado["name"].isna().sum(), 0)
        self.assertEqual(resultado["age"].isna().sum(), 0)

        self.assertLess(len(resultado), len(DataFrame))
    ...
    def test_drop_invalid_rows_raises_keyerror_for_unknown_column(self):
        ...
        with self.assertRaises(KeyError):
            cleaner.drop_invalid_rows(DataFrame, ["does_not_exist"])
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        DataFrame_Original = DataFrame.copy(deep=True)
        resultado = cleaner.trim_strings(DataFrame, ["name"])

        self.assertEqual(DataFrame.loc[0, "name"], " Alice ")
        self.assertEqual(DataFrame.loc[3, "name"], " Carol  ")

        self.assertEqual(resultado.loc[0, "name"], "Alice")
        self.assertEqual(resultado.loc[3, "name"], "Carol")

        pdt.assert_series_equal(resultado["city"], DataFrame_Original["city"])
    ...
    def test_trim_strings_raises_typeerror_for_non_string_column(self):
        ...
        with self.assertRaises(TypeError):
            cleaner.trim_strings(DataFrame, ["age"])
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        resultado = cleaner.remove_outliers_iqr(DataFrame, "age", factor=0.5)
    ...
    def test_remove_outliers_iqr_raises_keyerror_for_missing_column(self):
        ...
        with self.assertRaises(KeyError):
            cleaner.remove_outliers_iqr(DataFrame, "salary")
    ...
    def test_remove_outliers_iqr_raises_typeerror_for_non_numeric_column(self):
        ...
        with self.assertRaises(TypeError):
            cleaner.remove_outliers_iqr(DataFrame, "city")
