...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        resultado = utils.moving_average(lista, window)

        npt.assert_allclose(resultado, expected, rtol=1e-7, atol=1e-7)
        self.assertEqual(resultado.shape, expected.shape)
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        # Ventana mayor que la longitud de la lista
        with self.assertRaises(ValueError):
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        utils = StatisticsUtils()
        lista = [10, 20, 30, 40]
        resultado = utils.zscore(lista)

        #Media aproximadamente 0
        self.assertAlmostEqual(np.mean(resultado), 0.0, places = 7)

        # Desviación estandar aproximadamente 1
        self.assertAlmostEqual(np.std(resultado), 1.0, places=7)
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        utils = StatisticsUtils()
        lista_valores_iguales = [7, 7, 7]

        with self.assertRaises(ValueError):
            utils.zscore(lista_valores_iguales)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        utils = StatisticsUtils()
        lista = [2, 4, 6]
        resultado = utils.min_max_scale(lista)

        expected = np.array([0.0, 0.5, 1.0])

        # Verificar valor minimo = 0.0
        self.assertAlmostEqual(resultado.min(), 0.0, places=7)

        # Verificar valor maximo = 1.0
        self.assertAlmostEqual(resultado.max(), 1.0, places=7)

        # Verificar valores transformados
        npt.assert_allclose(resultado, expected, rtol=1e-7, atol=1e-7)
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        utils = StatisticsUtils()
        lista_valores_iguales = [4, 4, 4]

        with self.assertRaises(ValueError):
            utils.min_max_scale(lista_valores_iguales)
